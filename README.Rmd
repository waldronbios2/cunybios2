...
dt <- data.frame(
  session = cell_spec(
    seq_along(chapters),
    tooltip = sapply(chapters, function(x) packageDescription(x)$Description),
    link = paste0("https://waldronbios2.github.io/", chapters)),
  git = cell_spec(
    "git",
    tooltip = "use for git clone",
...
kbl(dt, "html", escape = FALSE, col.names = NULL, align="c") %>%
  kable_paper() %>%
  kable_styling(bootstrap_options = "striped", full_width = FALSE, position = "center") %>%
  add_header_above(c("Session" = 1, "Sites" = 1, "Lecture" = 2, "Lab" = 1))
```
